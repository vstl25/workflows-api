...
const autoAdvanceItemUntilApproval = async (
  client: any,
  itemId: string,
  ...
  while (true) {
    const approvalTransitions = currentStateTransitions.filter((row: any) => row.requires_approval);
    const nonApprovalTransitions = currentStateTransitions.filter((row: any) => !row.requires_approval);

    if (approvalTransitions.length > 0) {
      // Pick a single approval-required transition originating from the current state.
      const row = approvalTransitions.find((r: any) => r.from_state_id === currentStateId) || approvalTransitions[0];
      ...
      if (existing) {
        ...
        const r = await repo.insertApprovalRequest(
          client,
          tenantId,
          itemId,
          row.transition_id,
...
export const itemService = {
  raiseRequest: async (actorId: string, payload: any) => {
    ...
    if (role !== "viewer") {
      const err: any = new Error("You are unauthorized to raise a request");
      err.status = 403;
      throw err;
    }
    ...
    if (!initialStateId) {
      const err: any = new Error("No initial state found for workflow");
      err.status = 400;
      throw err;
    }
    ...
    try {
      ...
      const { approvals, finalStateId } = await autoAdvanceItemUntilApproval(
        client,
        item.id,
        initialStateId,
        tenant_id,
  ...
  cancelApprovalRequests: async (actorId: string, itemId: string) => {
    const result = await repo.cancelApprovalRequests(itemId);
    return { cancelled: result.length, requests: result };
  },

  voteApprovalRequest: async (actorId: string, requestId: string, payload: any) => {
    ...
    try {
      ...
      if (requestStatus !== "pending") {
        const err: any = new Error("Approval request is not pending");
        err.status = 400;
        throw err;
      }

      if (request.requested_by === actorId) {
        const err: any = new Error("You cannot approve your own request");
        err.status = 403;
        throw err;
      }
      ...
      if (role !== "approver") {
        const err: any = new Error("Only approvers may vote on approval requests");
        err.status = 403;
        throw err;
      }
  ...
  getPendingItemsForApprover: async (userId: string, tenantId: string) => {
    const role = (await repo.getUserTenantRole(userId, tenantId))?.toLowerCase();
    if (role !== "approver") {
      const err: any = new Error("Only approvers can view pending items");
      err.status = 403;
      throw err;
    }
  ...
  getMyApprovalRequests: async (userId: string, tenantId: string) => {
    const role = (await repo.getUserTenantRole(userId, tenantId))?.toLowerCase();
    if (role !== "viewer") {
      const err: any = new Error("Only viewers can view their approval requests");
      err.status = 403;
      throw err;
    }
