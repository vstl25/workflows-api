...
export const workflowService = {
  createWorkflow: async (actorId: string, payload: any) => {
    ...
    if (!role || role !== "admin") {
      const err: any = new Error("You are unauthorized");
      err.status = 403;
      throw err;
    }

    // Basic validations beyond Joi
    const initialCount = states.filter((s: any) => !!s.is_initial).length;
    if (initialCount !== 1) throw new Error("Exactly one state must be initial");

    const stateNames = new Set(states.map((s: any) => s.name));
    if (stateNames.size !== states.length) throw new Error("State names must be unique");
    ...
    for (const t of transitions) {
      ...
      if (t.requires_approval) {
        if (!t.policy) throw new Error(`Transition '${t.name}' requires a policy`);
    ...
    try {
      ...
      for (let i = 0; i < transitions.length; i++) {
        ...
        if (t.sla_rule) {
          await repo.insertSlaRule(
            client,
            tenant_id,
            workflowDefId,
            toId,
            t.sla_rule.name,
            t.sla_rule.duration_hours,
            t.sla_rule.escalation_action,
            t.sla_rule.escalation_target ?? null
          );
        }
      }
      ...
      // audit log
      await client.query(
        `INSERT INTO audit_logs (tenant_id, actor_id, action, entity_type, entity_id, new_value) VALUES ($1,$2,'workflow.created','workflow_definition',$3,$4)`,
        [tenant_id, actorId, workflowDefId, JSON.stringify({ workflow, states, transitions })]
      );
  ...
  listWorkflows: async (userId: string, tenantId: string) => {
    const workflows = await repo.getWorkflowsByCreatorAndTenant(userId, tenantId);
    return workflows;
  },
  getWorkflowsForDropdown: async (tenantId: string) => {
