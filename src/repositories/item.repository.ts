...
export const getUserTenantRole = async (userId: string, tenantId: string) => {
 try {
...
export const getInitialStateId = async (workflowDefId: string) => {
  const q = `
    SELECT id
...
export const getApprovalTransitionsFromState = async (stateId: string) => {
  const q = `
...
export const getOutgoingTransitionsFromState = async (stateId: string) => {
  const q = `
    SELECT wt.id AS transition_id, wt.workflow_def_id, wt.from_state_id, wt.to_state_id, wt.name, wt.requires_approval,
           ap.id AS policy_id, ap.strategy, ap.quorum_count, ap.timeout_hours
    FROM workflow_transitions wt
...
export const insertApprovalRequest = async (
  client: any,
  tenantId: string,
  itemId: string,
  transitionId: string,
...
export const getPendingApprovalRequest = async (itemId: string, transitionId: string, policyId: string) => {
  const q = `
    SELECT id
...
export const findRecentItemByUser = async (
  tenantId: string,
  workflowDefId: string,
  title: string,
  createdBy: string,
...
export const getPendingApprovalsForItem = async (itemId: string) => {
  const q = `
...
export const getApprovalRequestById = async (requestId: string) => {
  const q = `
...
export const getApprovalRequestByIdForUpdate = async (client: any, requestId: string) => {
  const q = `
    SELECT ar.id, ar.item_id, ar.tenant_id, ar.requested_by, ar.status, ar.policy_id,
           ap.strategy, ap.quorum_count
    FROM approval_requests ar
...
export const insertApprovalVote = async (
  client: any,
  requestId: string,
  approverId: string,
  decision: string,
...
export const getApprovalVoteCounts = async (requestId: string) => {
  const q = `
...
export const updateApprovalRequestStatus = async (client: any, requestId: string, status: string) => {
  const q = `
...
export const insertSlaBreach = async (client: any, tenantId: string, slaRuleId: string, itemId: string) => {
  const q = `
...
export const cancelApprovalRequests = async (itemId: string) => {
  const q = `
    UPDATE approval_requests
...
export const getTransitionToStateId = async (client: any, transitionId: string) => {
  const q = `
    SELECT to_state_id
...
export const updateItemCurrentState = async (client: any, itemId: string, stateId: string) => {
  const q = `
    UPDATE items
...
export const getPendingItemsForApprover = async (tenantId: string, approverId: string) => {
  const q = `
    SELECT DISTINCT
      i.id AS item_id,
...
export const getMyApprovalRequests = async (tenantId: string, userId: string) => {
  const q = `
    SELECT
      ar.id AS request_id,
      ar.item_id,
